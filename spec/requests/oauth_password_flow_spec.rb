RSpec.describe "OAuth password flow" do
  ...
  it "creates a token and returns it when the credentials are valid and get /v1/users/me" do
    post "/oauth/token", params: { grant_type: "password", username: email_address, password: password }
    expect(response.status).to eq 200

    access_token = JSON.parse(response.body)["access_token"]
    expect(access_token).not_to be_nil

    get "/v1/users/me", headers: { "Content-Type": "application/vnd.api+json", "Authorization": "Bearer #{access_token}" }
    expect(response.status).to eq 200
